 ...
 *
 * Hoisted copy (2026-09-16) of `packages/templates/src/m0saic/_shared/
 * onboarding-cover.ts` — the original stays put for the frozen first-party
 * templates; this copy is the one a template repo that may only import
 * `@m0saic/template-utils` reaches (community lyric-video was the first).
 * Dependency direction is templates → template-utils, so the kit's closure
 * (screencap-grid cover + tutorial + assets, the dark theme preset) rides
 * along as siblings in this folder; the code is otherwise unchanged.
 *
 ...
 *
 * The theme contract (the mosaic-branding-covers ruling):
