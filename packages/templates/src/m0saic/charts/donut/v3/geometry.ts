/**
 * ============================================================================
 * @m0saic/charts/donut/v3 — ring geometry (pure)
 * ============================================================================
 *
 * Pure math for the donut's annular sectors. No engine imports, no I/O —
 * deterministic functions a unit test can pin exactly.
 *
 * v3 adds `annularSectorBBox`: the TIGHT axis-aligned bounding box of a sector,
 * so each slice/sliver is placed as a small rect (placeRects) with a cell-local
 * mask — instead of v1's full-canvas mask per slice. A thin sliver then paints
 * into a thin intermediate, not a full 360×360 frame.
 *
 * Conventions:
...
/** Axis-aligned bounding box in absolute px. */
...
/**
 * Tight axis-aligned bbox of an annular sector. The extent is set by the four
 * corners (outer/inner × start/end) PLUS any cardinal the OUTER arc sweeps
 * through (90°/180°/270° → right/bottom/left extremes; the top extreme at 0°
 * is always a corner here since spans don't wrap past 0). The inner arc bulges
 * toward the center, so it never extends the box beyond the outer arc.
 */
export function annularSectorBBox(opts: AnnularSectorOpts): BBox {
  const { cx, cy, rOuter, rInner, startDeg, endDeg } = opts;
  const pts: Array<[number, number]> = [
    pt(cx, cy, rOuter, startDeg),
    pt(cx, cy, rOuter, endDeg),
    pt(cx, cy, rInner, startDeg),
    pt(cx, cy, rInner, endDeg),
  ];
  for (const c of [0, 90, 180, 270, 360]) {
    if (c > startDeg && c < endDeg) pts.push(pt(cx, cy, rOuter, c));
  }
