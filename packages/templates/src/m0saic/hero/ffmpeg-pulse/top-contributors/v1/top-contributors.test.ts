...
import { FfmpegPulseTopContributors } from "./top-contributors";
import "../../../../alpine/contributor-table/v1"; // side-effect: register the nested table
...
function makeCtx(W: number, H: number, durationMs = 9000): MosaicEngineContext {
  return {
    ...
    output: { width: W, height: H, fps: 30, durationMs, workspaceDir: "/tmp/top-contributors" },
    media: {},
  } as unknown as MosaicEngineContext;
}
const render = (W: number, H: number, props: Record<string, unknown> = {}) =>
  FfmpegPulseTopContributors.render({ ...(FfmpegPulseTopContributors.defaultProps as object), ...props } as never, makeCtx(W, H)) as Promise<MosaicDocument>;
...
describe("FfmpegPulseTopContributors — metadata", () => {
  it("is the registered internal Top Contributors beat", () => {
    expect(FfmpegPulseTopContributors.id).toBe("@m0saic/hero/ffmpeg-pulse/top-contributors/v1");
    expect(FfmpegPulseTopContributors.version).toBe(1);
    expect(FfmpegPulseTopContributors.internal).toBe(true);
    expect((FfmpegPulseTopContributors as { deprecated?: unknown }).deprecated).toBeUndefined();
    expect(FfmpegPulseTopContributors.tags).toEqual(expect.arrayContaining(["hero", "ffmpeg-pulse", "top-contributors"]));
  });
});

describe("FfmpegPulseTopContributors — renders at every aspect", () => {
  for (const [W, H, label] of ASPECTS) {
    it(`${label} ${W}x${H}: valid m0, nested table + 4 rail tiles, header + footer copy`, async () => {
      ...
      // One contributor table + one stat card per aggregate rail tile (4).
      expect(childKeys(doc)).toEqual(["rail0", "rail1", "rail2", "rail3", "table"]);
      ...
      const t = texts(doc);
      expect(t).toEqual(expect.arrayContaining(["TOP CONTRIBUTORS", "BEAT 3"]));
...
describe("FfmpegPulseTopContributors — layout contract holds across resolutions", () => {
