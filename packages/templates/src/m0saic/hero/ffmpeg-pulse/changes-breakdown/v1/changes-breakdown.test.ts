...
import { FfmpegPulseChangesBreakdown } from "./changes-breakdown";
import "../../../../alpine/donut/v1"; //      side-effect: register the nested donut
...
function makeCtx(W: number, H: number, durationMs = 9000): MosaicEngineContext {
  return {
    ...
    output: { width: W, height: H, fps: 30, durationMs, workspaceDir: "/tmp/changes-breakdown" },
    media: {},
  } as unknown as MosaicEngineContext;
}
const render = (W: number, H: number, props: Record<string, unknown> = {}) =>
  FfmpegPulseChangesBreakdown.render({ ...(FfmpegPulseChangesBreakdown.defaultProps as object), ...props } as never, makeCtx(W, H)) as Promise<MosaicDocument>;
...
describe("FfmpegPulseChangesBreakdown — metadata", () => {
  it("is the registered internal Changes Breakdown beat", () => {
    expect(FfmpegPulseChangesBreakdown.id).toBe("@m0saic/hero/ffmpeg-pulse/changes-breakdown/v1");
    expect(FfmpegPulseChangesBreakdown.version).toBe(1);
    expect(FfmpegPulseChangesBreakdown.internal).toBe(true);
    expect((FfmpegPulseChangesBreakdown as { deprecated?: unknown }).deprecated).toBeUndefined();
    expect(FfmpegPulseChangesBreakdown.tags).toEqual(expect.arrayContaining(["hero", "ffmpeg-pulse", "changes-breakdown"]));
  });
});

describe("FfmpegPulseChangesBreakdown — renders at every aspect", () => {
  for (const [W, H, label] of ASPECTS) {
    it(`${label} ${W}x${H}: valid m0, nested donut + 4 rail tiles, header + footer copy`, async () => {
      ...
      // One donut + one stat card per aggregate rail tile (4).
      expect(childKeys(doc)).toEqual(["donut", "rail0", "rail1", "rail2", "rail3"]);
      ...
      const t = texts(doc);
      expect(t).toEqual(expect.arrayContaining(["CHANGES BREAKDOWN", "BEAT 4"]));
...
describe("FfmpegPulseChangesBreakdown — layout contract holds across resolutions", () => {
