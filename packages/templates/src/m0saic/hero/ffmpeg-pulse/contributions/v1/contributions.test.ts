...
import { FfmpegPulseContributions } from "./contributions";
import "../../../../alpine/heatmap/v1"; //    side-effect: register the nested heatmap
...
function makeCtx(W: number, H: number, durationMs = 9000): MosaicEngineContext {
  return {
    ...
    output: { width: W, height: H, fps: 30, durationMs, workspaceDir: "/tmp/contributions" },
    media: {},
  } as unknown as MosaicEngineContext;
}
const render = (W: number, H: number, props: Record<string, unknown> = {}) =>
  FfmpegPulseContributions.render({ ...(FfmpegPulseContributions.defaultProps as object), ...props } as never, makeCtx(W, H)) as Promise<MosaicDocument>;
...
describe("FfmpegPulseContributions — metadata", () => {
  it("is the registered internal Contributions beat", () => {
    expect(FfmpegPulseContributions.id).toBe("@m0saic/hero/ffmpeg-pulse/contributions/v1");
    expect(FfmpegPulseContributions.version).toBe(1);
    expect(FfmpegPulseContributions.internal).toBe(true);
    expect((FfmpegPulseContributions as { deprecated?: unknown }).deprecated).toBeUndefined();
    expect(FfmpegPulseContributions.tags).toEqual(expect.arrayContaining(["hero", "ffmpeg-pulse", "contributions", "heatmap"]));
  });
});

describe("FfmpegPulseContributions — renders at every aspect", () => {
  for (const [W, H, label] of ASPECTS) {
    it(`${label} ${W}x${H}: valid m0, nested heatmap + 4 rail tiles, header + footer copy`, async () => {
      ...
      // One heatmap + one stat card per aggregate rail tile (4).
      expect(childKeys(doc)).toEqual(["heat", "rail0", "rail1", "rail2", "rail3"]);
      ...
      const t = texts(doc);
      expect(t).toEqual(expect.arrayContaining(["CONTRIBUTIONS", "BEAT 5"]));
...
describe("FfmpegPulseContributions — layout contract holds across resolutions", () => {
